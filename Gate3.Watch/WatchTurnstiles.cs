        ...
        void displayRecordInformation(byte[] recv)
        {
            ...
            if (recordCardNO != null && recordCardNO.Count() >= 10)
            {
                ...
                if (DateTime.Now.Hour < 18)
                {
                    ...
                    Ticket itick = db.Tickets.Where(x => x.TicketNo == recordCardNO && EntityFunctions.TruncateTime((DateTime)x.CreatedDate) == todayDate && x.Status == false).OrderByDescending(x => x.Id).FirstOrDefault();
...
"45","RemoteOpen","Remote Open Door By USB Reader","发卡器确定发出的远程开门"
        ...
        {
            GateEntities db = new GateEntities();
            var server = db.TurnStileServers.Find(3);
